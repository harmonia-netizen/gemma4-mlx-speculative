...
結果として、**エージェント用途などの「共通の長文履歴を持ち、短く予測可能なコマンドを出力する」シナリオ** において、極めて効果的な高速化アーキテクチャ (Template Draft Runtime) を構築できることが証明された。
...
3. **統合ランタイム (Template Draft Runtime)**
   ...
   - 統合による副作用 (微小な Logit の差異による Token Mismatch) も、チャンク処理の境界を Baseline と統一することで完全に解決し、実用的なアーキテクチャの Prototype が完成した。
...
- **Token Match**: すべての検証ケース (A. Baseline, B. Prefix Reuse, C. Prefix Reuse + Draft) において、浮動小数点の計算順序差異を補正した上で **完全一致** を達成。
