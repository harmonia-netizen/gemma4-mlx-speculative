...
def clone_cache_state_full(prompt_cache):
    snapshot = []
    for c in prompt_cache:
        state = getattr(c, "state", None)
        ...
        if state is None:
            snapshot.append((None, meta_state))
            continue

        snapshot.append((clone_value(state), meta_state))
    return snapshot


def restore_cache_state_full(prompt_cache, snapshot):
    if len(prompt_cache) != len(snapshot):
        raise RuntimeError("cache/snapshot length mismatch")

    for i, (c, (state, meta_state)) in enumerate(zip(prompt_cache, snapshot)):
        if state is not None:
            c.state = clone_value(state)

        if meta_state is not None:
            if not hasattr(c, "meta_state"):
                raise RuntimeError(f"cache[{i}] has no meta_state setter")
            c.meta_state = meta_state

    eval_cache(prompt_cache)


def verify_full_state_restore(lm, prompt_cache, next_logits, dtype, label):
    snap = clone_cache_state_full(prompt_cache)

    tok = argmax_token(next_logits)
    logits_a = forward_one(lm, tok[:, None], prompt_cache)

    restore_cache_state_full(prompt_cache, snap)
    logits_b = forward_one(lm, tok[:, None], prompt_cache)

    restore_cache_state_full(prompt_cache, snap)

    a = argmax_id(logits_a)
    b = argmax_id(logits_b)

    diff = mx.max(mx.abs(logits_a - logits_b))
    mx.eval(diff)
    diff_value = float(diff.item())

    if a != b:
        raise RuntimeError(
            f"{label}: full cache restore argmax mismatch: {a} != {b}"
        )

    if diff_value > 1e-2:
        raise RuntimeError(
            f"{label}: full cache restore logits mismatch: {diff_value}"
        )

    print(f"full cache restore check OK: {label}; argmax={a}; max_abs_diff={diff_value:.6f}")
...
def run_speculative(
    ...
    while len(out) < max_tokens:
        ...
        target_snapshot = clone_cache_state_full(target_cache)
        draft_snapshot = clone_cache_state_full(draft_cache)
        ...
        restore_cache_state_full(target_cache, target_snapshot)
        restore_cache_state_full(draft_cache, draft_snapshot)
