...
def template_draft_text(user_prompt):
    ...
    if "3行のbashブロック" in p and "pytest" in p:
        return """```bash
git status --short
pytest --tb=short
git diff
...
def run_speculative(
    ...
    while len(out) < max_tokens:
        ...
        cloned_cache = clone_cache_objects(target_cache)
        verify_next_logits = target_next_logits
        ...
        for i, proposed_id in enumerate(proposal_ids):
            target_tok = argmax_token(verify_next_logits)
            ...
            if target_id != proposed_id:
                ...
                rejected += 1
                if trace_template:
                    print(f"trace: reject at block idx {i}, target_id={target_id} {repr(decode_text(tokenizer, [target_id]))}, proposed_id={proposed_id} {repr(decode_text(tokenizer, [proposed_id]))}")
                break
            ...
            verify_next_logits = forward_one(
                target_lm,
                mx.array([[proposed_id]], dtype=first.dtype),
                cloned_cache,
            )
