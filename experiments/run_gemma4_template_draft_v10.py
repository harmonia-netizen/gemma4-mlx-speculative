...
def template_draft_text(user_prompt):
    p = user_prompt.lower()

    if (
        ("3行" in p or "3 commands" in p or "3コマンド" in p)
        and ("pytest" in p)
        and ("手順" in p or "plan" in p or "確認" in p)
    ):
        return """```bash
git status --short
pytest --tb=short
...
def run_speculative(
    ...
    candidate_text = template_draft_text(user_prompt)
    candidate_ids = encode_template_candidate(tokenizer, candidate_text)

    # The first token is already generated by the target. Drop it from the
    # template candidate if it matches the already emitted stream.
    if candidate_ids and candidate_ids[0] == first_id:
        candidate_ids = candidate_ids[1:]

    # Gating:
    # Short template candidates lose to target greedy because verify/restore/commit
    # overhead dominates. Use template draft only when enough tokens remain.
    ...
    while len(out) < max_tokens:
        ...
        if not proposal_ids:
            # Template exhausted or unavailable. Continue with target greedy.
        ...
        for i, proposed_id in enumerate(proposal_ids):
        ...
        if target_next_logits is None:
            break

        mx.eval(target_next_logits)

        if reached_stop:
            break

        if mismatch:
            # Template no longer matches target. Fall back to target greedy.
            candidate_ids = []
            cursor = 0
