...
V14_PATH = Path(__file__).with_name("run_gemma4_template_draft_v14.py")
...
def load_v14_module():
    spec = importlib.util.spec_from_file_location("template_draft_v14", V14_PATH)
...
def main():
    ...
    model_path = args.model or v14.DEFAULT_TARGET_MODEL_PATH

    print("loading target:", model_path)
    target_model, processor = v14.d.load(model_path)

    tokenizer = getattr(processor, "tokenizer", processor)
    stop_ids = v14.build_stop_ids(tokenizer)
    ...
    for case in selected_cases(args.case):
        ...
        target_prompt = v14.format_prompt(processor, case.prompt)
