...
V11_PATH = Path(__file__).with_name("run_gemma4_template_draft_v11.py")
...
def load_v9_module():
    spec = importlib.util.spec_from_file_location("template_draft_v11", V11_PATH)
...
def main():
    ...
    model_path = args.model or v9.DEFAULT_TARGET_MODEL_PATH

    print("loading target:", model_path)
    target_model, processor = v9.d.load(model_path)

    tokenizer = getattr(processor, "tokenizer", processor)
    stop_ids = v9.build_stop_ids(tokenizer)
    ...
    for case in selected_cases(args.case):
        ...
        target_prompt = v9.format_prompt(processor, case.prompt)
